...
class DataConfigFactory(abc.ABC):
    ...
    def create_base_config(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        return dataclasses.replace(
            ...
            # use_quantile_norm=model_config.model_type != ModelType.PI0, # 这是原本的
...
class LeRobotSo100DataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # replace the transforms below with your own.
        data_transforms = _transforms.Group(
            inputs=[so100_policy.S0100Inputs(model_type=model_config.model_type, action_dim=model_config.action_dim)],
...
_CONFIGS = [
    ...
    # 在_CONFIGS列表中新增
    TrainConfig(
        name="pi05_so100",  # 唯一配置名，用于调用
        # 复用pi05_libero的模型配置，仅调整action_dim为so100的6维
        model=pi0_config.Pi0Config(
            pi05=True, 
            # action_dim=32,        # 核心修改：匹配so100的6维动作（原Libero是7维）
            action_horizon=10,   # 保持pi05的动作窗口长度
            max_token_len=180,
    ...
    TrainConfig(
        ...
        model=pi0_config.Pi0Config(
            ...
            action_dim=32,  # 核心适配：SO100 机器人的6维动作空间（替换原Libero的7维）
